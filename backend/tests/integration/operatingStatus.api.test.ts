...
describe('GET /api/v1/centers/:id/operating-status', () => {
  ...
  describe('Success Cases', () => {
    it('should return 200 with operating status data', async () => {
      ...
      expect(mockService.calculateOperatingStatus).toHaveBeenCalledWith(1, expect.any(Date));
    ...
    it('should return cached data when available (cache hit)', async () => {
      ...
      // Make request
      const response = await request(app).get('/api/v1/centers/1/operating-status').expect(200);
  ...
  describe('Validation Errors', () => {
    ...
    it('should return 400 for invalid center ID (negative)', async () => {
      const response = await request(app).get('/api/v1/centers/-1/operating-status').expect(400);
  ...
  describe('Not Found Errors', () => {
    it('should return 404 when center does not exist', async () => {
      ...
      const response = await request(app).get('/api/v1/centers/999/operating-status').expect(404);
  ...
  describe('Server Errors', () => {
    it('should return 500 for unexpected errors', async () => {
      ...
      const response = await request(app).get('/api/v1/centers/1/operating-status').expect(500);
    ...
    it('should handle cache read failures gracefully', async () => {
      ...
      mockCache.getCachedOperatingStatus.mockRejectedValue(new Error('Redis connection timeout'));
      mockCache.cacheOperatingStatus.mockResolvedValue(false);
      mockService.calculateOperatingStatus.mockResolvedValue(mockOperatingStatusResponse);

      const response = await request(app).get('/api/v1/centers/1/operating-status').expect(500); // Will fail due to unhandled cache error
