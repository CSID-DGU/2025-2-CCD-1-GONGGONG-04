...
async function seedRecommendationTestData() {
  ...
  const center2 = await prisma.center.create({
    data: {
      ...
      isActive: true,
      staff: {
        create: [{ staffType: 'nurse', staffCount: 2 }],
      },
    },
  });
  ...
  const center3 = await prisma.center.create({
    data: {
      ...
      isActive: true,
      staff: {
        create: [{ staffType: 'social_worker', staffCount: 1 }],
      },
    },
  });
...
describe('POST /api/v1/recommendations/calculate', () => {
  ...
  describe('Validation Errors (400 Bad Request)', () => {
    ...
    test('should reject invalid maxDistance (> 100)', async () => {
      const requestBody = {
        sessionId: 'test-session-123',
        location: {
          latitude: 37.5665,
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle boundary latitude values (-90, 90)', async () => {
      ...
      await request(app).post('/api/v1/recommendations/calculate').send(requestBody1).expect(200);
      ...
      await request(app).post('/api/v1/recommendations/calculate').send(requestBody2).expect(200);
    });
  ...
  describe('Performance Tests', () => {
    test('should respond within performance threshold (<500ms)', async () => {
      ...
      await request(app).post('/api/v1/recommendations/calculate').send(requestBody).expect(200);
